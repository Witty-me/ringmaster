...
{
  cerr <<
  "Usage: " << program_name << " [options] host port y4m width height\n\n"
  "Options:\n"
...
int main(int argc, char * argv[])
{
  ...
  const auto width = narrow_cast<uint16_t>(strict_stoi(argv[optind + 3]));
  const auto height = narrow_cast<uint16_t>(strict_stoi(argv[optind + 4]));

  Address peer_addr{host, port};
  cerr << "Peer address: " << peer_addr.str() << endl;

  // create a UDP socket and "connect" it to the peer (receiver)
  UDPSocket udp_sock;
